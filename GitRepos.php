 ...
 * @Last Modified time: 2018-05-11 09:35:40
 */
...
class GitRepo
{
    ...
    public function Count_Commits()
    {
        try {
            $this->fetch();
            $b = $this->active_branch();
            $this->checkout($b);
            $this->run("rev-parse " . $b);
            $rp = $this->run("rev-parse --symbolic-full-name " . $b . "@{u}");
            $output = $this->run("rev-list --left-right " . $b . "..." . $rp);
            $in = substr_count($output, ">");
            $out = substr_count($output, "<");
            return array('down' => $in, 'up' => $out);
        } catch (Exception $ex) {
            return array('down' => "Err", 'up' => "Err");
        }
    }
